	...
	function timelord_shortcode( $atts, $content = null ) {
		...
		if( !empty($a['year']) ) { // [timelord year="YEAR" ordinal="yes"]
			...
			*/
			...
			$interval 	= DateInterval::createFromDateString($a['every']);
			$period 	= new DatePeriod($begin, $interval, $end, DatePeriod::EXCLUDE_START_DATE);
			$i 			= 0;

			foreach ( $period as $dt ) :
				$count_interval 	= strtotime($a['every']) - $now;
				$count_iteration 	= strtotime($dt->format( "l Y-m-d H:i:s"));
				$count_difference 	= $count_iteration - $now;
				
				if( ($i++ % 2 == 0) && ($count_difference < $count_interval) && ($count_difference > 0)  ) : // If period is even (not odd), it has began but its not over yet
